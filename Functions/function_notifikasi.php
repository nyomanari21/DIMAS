...
// fungsi menampilkan data notifikasi
function show_notifikasi()
{
    global $koneksi;

    // query sql
    $sql = "SELECT barang.nama_barang, barang.lifetime, log_barang.tanggal_input, DATE_ADD(log_barang.tanggal_input,
            INTERVAL (barang.lifetime) DAY) AS tanggal_pemeliharaan, DATE_ADD(log_barang.tanggal_input,
            INTERVAL (barang.lifetime - 7) DAY) AS tanggal_notifikasi FROM barang
